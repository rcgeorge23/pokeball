...
test('getBattleDamageMultiplier boosts player damage early and tapers over time', () => {
  assert.equal(getBattleDamageMultiplier('player', 0), 1.45);
  assert.ok(Math.abs(getBattleDamageMultiplier('player', 4) - 1.225) < 1e-9);
...
test('getBattleDamageMultiplier reduces opponent damage early and restores it over time', () => {
  assert.equal(getBattleDamageMultiplier('opponent', 0), 0.6);
  assert.equal(getBattleDamageMultiplier('opponent', 4), 0.8);
